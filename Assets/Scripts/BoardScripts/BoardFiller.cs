...
namespace BoardScripts
{
    ...
    public class BoardFiller : MonoBehaviour
    {
        ...
        private GamePiece FillRandomGamePieceAt(int x, int y, int falseYOffset = 0, float moveTime = 0.1f)
        {
            ...
            {
                GameObject randomPiece = Instantiate(board.boardQuery.GetRandomGamePiece(), Vector3.zero, Quaternion.identity);
        ...
        private GamePiece FillRandomCollectibleAt(int x, int y, int falseYOffset = 0, float moveTime = 0.1f)
        {
            ...
            if (board.boardQuery.IsWithinBounds(x, y))
            {
                GameObject randomPiece = Instantiate(board.boardQuery.GetRandomCollectible(), Vector3.zero, Quaternion.identity);
                MakeGamePiece(randomPiece, x, y, falseYOffset, moveTime);
                return randomPiece.GetComponent<GamePiece>();
            }
            return null;
        ...
        public void MakeTile(GameObject prefab, int x, int y, int z = 0)
        {
            ...
            if (prefab != null && board.boardQuery.IsWithinBounds(x, y))
            {
                ...
                tile.name = "Tile (" + x + "," + y + ")";
                board.allTiles[x, y] = tile.GetComponent<Tile.Tile>();
