...
namespace BoardScripts
{
    ...
    public class BoardBomber : MonoBehaviour
    {
        ...
        public void ProcessBombs(Tile.Tile tileA, Tile.Tile tileB, GamePiece clickedPiece, GamePiece targetPiece, List<GamePiece> tileAPieces, List<GamePiece> tileBPieces)
        {
        ...
        private GameObject DropBomb(int x, int y, Vector2 swapDirection, List<GamePiece> gamePieces)
        {
            ...
            if (gamePieces is { Count: >= 5 } && matchValue != MatchValue.None)
            {
            ...
            else if (gamePieces is { Count: 4 } && matchValue != MatchValue.None)
            {
        ...
        {
            var bombTransformPosition = bomb.transform.position;
            int x = (int)bombTransformPosition.x;
            int y = (int)bombTransformPosition.y;
