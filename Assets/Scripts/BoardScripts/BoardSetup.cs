        ...
        public void SetupCamera()
        {
            ...
            if (Camera.main != null)
            {
                Camera.main.transform.position =
                    new Vector3((board.width - 1) / 2f, (board.height - 1) / 2f, -10f);
            
                float aspectRatio = (Screen.width) / (float)(Screen.height);

                float verticalSize = board.height / 2f + board.borderSize;

                float horizontalSize = (board.width / 2f + board.borderSize) / aspectRatio;
