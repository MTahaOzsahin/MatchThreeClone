...
namespace BoardScripts
{
    ...
    public class BoardQuery : MonoBehaviour
    {
        ...
        // get blocker adjacent to position (x,y) (does not include diagonals)
        ...
        public bool IsNextTo(Tile.Tile start, Tile.Tile end)
        {
            ...
            for (int i = 0; i < board.width; i++)
            {
                if (board.allGamePieces[i, row] != null)
                {
                    ...
                    if (collectibleComponent != null)
                    {
                        ...
                        if (!clearedAtBottomOnly || (collectibleComponent.clearedAtBottom))
                        {
